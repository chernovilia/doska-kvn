...
- **Rate-limit по IP за Cloudflare**: `trust proxy 1` при цепочке Cloudflare → Amvera ingress, вероятно, отдаёт IP узла Cloudflare, а не клиента. Тогда пользователи делят общий лимит. Исправляем через `CF-Connecting-IP` + лимит по `userId`.
