...
export async function adminListAds({ limit = 100, offset = 0, status = null } = {}) {
  const qs = new URLSearchParams();
  qs.set('limit', String(limit));
  qs.set('offset', String(offset));
  if (status) qs.set('status', status);
  return adminRequest(`/admin/ads?${qs.toString()}`);
}

export async function adminSetAdStatus(id, status) {
  return adminRequest(`/admin/ads/${id}/status`, {
    method: 'PATCH',
...
export async function adminGetAd(id) {
  return adminRequest(`/admin/ads/${id}`);
}
...
export async function adminGetModeration() {
  return adminRequest('/admin/moderation');
}

export async function adminSetModeration(autoApprove) {
  return adminRequest('/admin/moderation', {
    method: 'PATCH',
